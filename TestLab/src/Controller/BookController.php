...
class BookController extends AbstractController
{
    ...
    /**
     * @Route("/sendBook/{id}", name="send_book", methods={"GET"})
     */
    public function sendBook(MessageBusInterface $bus, Book $book):Response
    {
        if($this->getUser() == null)
        {
            return new RedirectResponse($this->generateUrl('startpage'));
        }
        $bookSms= new BookMessage();
        $bookSms->setId($book->getId());
        $bookSms->setName($book->getName());
        $bookSms->setAuthor($book->getAuthor());
        //$user = $this->getUser()->getUsername();
        $bus->dispatch(new \App\Message\Book($bookSms));
